    ...
    <div v-else>
      <div class="d-flex ">
        ...
        <div class="d-flex my-auto ml-4">
          <img v-for="c in page.champions" :key="`champ-${c}`"
            class="round mr-3" height="30" width="30"
            :src="`/assets/champ-avis/${c}.png`"
          />
        </div>
      </div>
      <h4>by {{ user.displayname }}</h4>
      <div class="d-flex mt-4 bg-runes"
      ...
      >
        <img
          ...
          class="mr-3"
        />
        <img v-for="r in page.primary.rows" :key="`rune-${r}`"
      ...
      >
        <img
          ...
          class="mr-3"
        />
        <img v-for="r in page.secondary.rows" :key="`rune-${r}`"
      ...
      >
        <img v-for="(r, i) in page.perks.rows"
          :key="`perk-${r}-${i}`"
...
export default {
  ...
  created: function() {
    ...
    Rest.getShare(this.ident).then((res) => {
